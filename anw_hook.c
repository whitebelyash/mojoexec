...
const int ANATIVEWINDOW_QUERY_TRANSFORM_HINT = 8;
const int HAL_TRANSFORM_NONE = 0;

static int     (*nw_query_orig)(const struct ANativeWindow* window,
                                int what, int* value);
...
static int hook_query(const struct ANativeWindow* window,
                      int what, int* value) {
    if(what == ANATIVEWINDOW_QUERY_TRANSFORM_HINT) {
        *value = HAL_TRANSFORM_NONE;
        return 0;
    }
    return nw_query_orig(window, what, value);
}

int mojoexec_override_pretransform(void* native_window) {
    if(!native_window || !hook_enable) return -1;
    struct ANativeWindow_real* nativeWindowReal = (struct ANativeWindow_real*) native_window;

    if(nativeWindowReal->common.magic != ANDROID_NATIVE_WINDOW_MAGIC) {
        return -1;
    }
    if(nativeWindowReal->query != hook_query) {
        nw_query_orig = nativeWindowReal->query;
        nativeWindowReal->query = hook_query;
    }
    return 0;
}

JNIEXPORT void JNICALL
Java_git_artdeell_mojoexec_MojoExec_setEnableTransformHook(JNIEnv *env, jclass clazz,
                                                           jboolean enable) {
    hook_enable = enable;
}